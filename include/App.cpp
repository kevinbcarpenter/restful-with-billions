...
void App::configUpdate( const json& config ) {
	...
	if ( dumpReset != Logger::dump( ) ) {
		...
		logEntry << "Dump packets reset: " + ( Logger::dump( ) ? string( "on" ) : string( "off" ) );
...
bool App::versionCheck( int argc, char* argv[], string version ) {
	if ( argc == 2 ) {
		...
		transform( versionCheck.begin( ), versionCheck.end( ), versionCheck.begin( ), ::toupper );
...
#ifdef STATE_SUPPORT
	configureStateSubscription( );	// State Subscription MUST be set before State Machine
...
//---------------------------------------------------------------------------
const json& App::getConfig( ) const {
	return configuration;
}

#ifndef NO_ZMQ_SUPPORT
//---------------------------------------------------------------------------
zmq::context_t& App::getContext( ) {
	return context;
}
#endif
